...
public:
	...
	Disk(const Properties &props) : Shape(props) {
		...
		m_spread = 1.0f - props.getFloat("spread", 1.0f);
		m_spread = std::max(0.0f, std::min(m_spread, 0.99f));

		std::string sampling = boost::to_lower_copy(props.getString("sampling", "area"));
//		std::string sampling = boost::to_lower_copy(props.getString("sampling", "polartab"));
//		std::string sampling = boost::to_lower_copy(props.getString("sampling", "polar"));
...
#endif
#if 0
		} else if (sampling == "cylindricaltab") {
			m_sampling_type = SamplingAlgorithm::SolidAngle;
#ifdef __DISK_DOUBLE_PRECISION
			m_sampling = DiskSampler<double>(SphericalEllipseSampler<double>::Type::CILINDRICTAB);
#else
			m_sampling = DiskSampler<Float>(SphericalEllipseSampler<Float>::Type::CILINDRICTAB);
#endif
#endif
		} else {
...
//#endif
	...
	void sampleDirect(DirectSamplingRecord &dRec, const Point2 &sample) const {
		...
		Point realRef = dRec.ref;
		Float d = dot((dRec.ref - c), n);
		if (m_spread > 0.f) {
			dRec.ref = dRec.ref - std::abs(d*m_spread - ShadowEpsilon)*n*((d > 0) ? 1.f : -1.f);
		}
		...
		if (m_sampling_type == SamplingAlgorithm::Area) {
			...
			/* Our paper main and single contribution starts here */
#ifdef __DISK_DOUBLE_PRECISION
			...
			dRec.dist = dRec.d.length();
			dRec.d /= dRec.dist;
			/* Our paper main and single contribution ends here */
		}
	...
	Float pdfDirect(const DirectSamplingRecord &dRec) const {
		...
		Float d = dot((dRec.ref - c), n);
		if (m_spread > 0.f) {
			lRec.ref = dRec.ref - std::abs(d*m_spread - ShadowEpsilon)*n*((d > 0) ? 1.f : -1.f);
		}
...
#else
				v3l::vector3<Float> o = v3l::vector3<Float>(lRec.ref.x, lRec.ref.y, lRec.ref.z);
				v3l::vector3<Float> cd = v3l::vector3<Float>(c.x, c.y, c.z);
				v3l::vector3<Float> nd = v3l::vector3<Float>(n.x, n.y, n.z);

				DiskSamplingRecord<Float> samplingRecord = m_sampling.createRecord(o, cd, nd, m_r);

				v3l::vector3<Float> p = v3l::vector3<Float>(lRec.p.x, dRec.p.y, lRec.p.z);
				Float pdf = samplingRecord.samplePdf(p);
#endif
				return pdf;
			} else if (lRec.measure == EArea) {
