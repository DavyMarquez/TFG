...
public:
	 ...
	 * 		r  : disk radius
	 */
	DiskSamplingRecord<T> createRecord(vector3<T>& o,
		const vector3<T>& cd, const vector3<T>& nd, T r) const;
};

template<class T> inline
DiskSamplingRecord<T> DiskSampler<T>::createRecord(vector3<T>& o,
	const vector3<T>& cd, const vector3<T>& nd, T r) const
{
	...
	/*for (int px = 100; px >= 0; px--) {
		for (int py = 100; py >= 0; py--) {
			for (int pz = 100; pz >= 0; pz--) {*/
				...
				if (length2(record.xd) < eps) {
					...
					if (std::abs(x) > std::abs(z)) {
						T invLength = T(1) / std::sqrt(x*x + y * y);
						record.xd = vector3<T>(-y * invLength, x*invLength, T(0));
					}
					else {
						T invLength = T(1) / std::sqrt(y*y + z * z);
						record.xd = vector3<T>(T(0), -z * invLength, y*invLength);
					}
				}
				else {
			...
			//	T x0_p = x0 / std::sqrt(x0*x0 + yh*yh + d*d);
				...
				cout << "shadPoint = [ " << o.x << ", " << o.y << ", " << o.z << " ]" << endl;*/
				//cout << "a = " << a << ", b = " << b << endl;
